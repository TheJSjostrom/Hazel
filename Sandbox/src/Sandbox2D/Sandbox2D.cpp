...
void Sandbox2D::OnUpdate(Hazel::Timestep ts)
{
	...
	m_Player.SetCollisionTest(CollisionTestPlayerTouch());
	m_Player.OnUpdate(ts, m_Triangle.Life);
...
void Sandbox2D::OnImGuiRender()
{
	...
	ImGui::ColorEdit4("Player color.", glm::value_ptr(m_Player.GetColor()));
...
bool Sandbox2D::OnKeyPressed(Hazel::KeyPressedEvent& e)
{
	if (e.GetKeyCode() == HZ_KEY_R)
	{
		...
		m_Player.SetProjectileCount(50);
	}
...
bool Sandbox2D::OnMouseButtonPressed(Hazel::MouseButtonPressedEvent& e)
{
	...
	m_Projectile[m_Index].Position.x = m_Player.GetPosition().x;
	m_Projectile[m_Index].Position.y = m_Player.GetPosition().y;
	m_Projectile[m_Index].Angle = m_Player.GetRotation();
	m_Index++;
...
bool Sandbox2D::CollisionTestPlayerTouch()
{
	glm::vec4 playerVertices[4] = {
		...
		for (int i = 0; i < 4; i++)
		{
			playerTransformedVerts[i] = glm::translate(glm::mat4(1.0f), m_Player.GetPosition())
				* glm::rotate(glm::mat4(1.0f), m_Player.GetRotation(), {0.0f, 0.0f, 1.0f})
				* glm::scale(glm::mat4(1.0f), m_Player.GetSize())
				* playerVertices[i];
		}
